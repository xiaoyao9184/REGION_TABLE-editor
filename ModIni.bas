...
'---------------------------------------------
Public Function GetINI(LpApplicationName As String, LpKeyName As String) As String
    Dim Retval As Long
    Dim Value As String
    Value = Space(128)
...
Public Sub WriteINI(LpApplicationName As String, LpKeyName As String, Value As String)
    Dim Retval As Long
    Retval = WritePrivateProfileString(LpApplicationName, LpKeyName, Value, Replace(App.Path & "\config.ini", "\\", "\"))
End Sub

Public Function GetType(LpApplicationName As String, LpKeyName As String, ByVal iType As String) As String
    Dim Retval As Long
    Dim Value As String
    Value = Space(128)
    Retval = GetPrivateProfileString(LpApplicationName, LpKeyName, "", Value, Len(Value), Replace(App.Path & "\MenuType\" & iType & ".txt", "\\", "\"))
